...
The supervised trainer can also filter rows at load time, so existing broad shards can be reused without writing derived NPZ files. `--min-row-turn`, `--max-row-turn`, `--require-contact`, `--min-visible-enemy-cells`, `--min-visible-enemy-density`, `--require-outcome-win`, `--require-search-best-win`, `--require-finish-within-250`, `--require-win-or-finish-within-250`, and `--min-search-score-gap` apply before `--max-samples-per-shard`, then the trainer prints kept/sampled row counts. This is useful for quick midgame decisive imitation probes; keep draw-heavy contrast rows in the same command only when action CE is gated off for those shard paths.
