...
`adaptive_plan_worker_supervised.py` 可从 Plan-Q shard 训练一个单独的 target-conditioned Worker：在保存的 adaptive observation 后追加 `source_one_hot`、`target_one_hot` 和 `route_potential` 三张 command plane，再用被选中 plan 的 primitive action 做监督。`evaluate_adaptive_policy.py --strategy-plan-worker-path <worker.eqx> --strategy-plan-worker-rerank-scale <x>` 会加载该 Worker，并把它的中心化 logits 作为 base strategy-spatial checkpoint 的小幅 bias。第一轮 best-plan v0 在 fixed-v5 max250 256-row 上有低 scale 正信号，同时 Expander 256-row min 基本持平；它是继续扩大数据的方向，不是 promotion 设置。
