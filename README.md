...
For multi-horizon finishability, use `--finish-head-mode multi-horizon`. This expands the strategy finish head to three independent BCE logits for `finish_within_50`, `finish_within_100`, and `finish_within_250`; warm-start old binary-finish checkpoints with `--init-finish-head-mode binary`. Evaluation should pass `--strategy-finish-outputs 3`. PPO warm-starts that only need to read and discard the old head can pass `--init-strategy-finish-outputs 3`; PPO runs that should preserve strategy heads in the output checkpoint must also pass `--strategy-aux --strategy-finish-outputs 3`.
...
The supervised trainer can also filter rows at load time, so existing broad shards can be reused without writing derived NPZ files. `--min-row-turn`, `--max-row-turn`, `--require-contact`, `--min-visible-enemy-cells`, `--min-visible-enemy-density`, `--require-outcome-win`, `--require-outcome-draw`, `--require-outcome-nonwin`, `--require-search-best-win`, `--require-finish-within-250`, `--require-win-or-finish-within-250`, and `--min-search-score-gap` apply before `--max-samples-per-shard`, then the trainer prints kept/sampled row counts. Combining `--require-outcome-draw` or `--require-outcome-nonwin` with `--require-search-best-win` isolates states where the recorded policy failed to win but rollout-search found a winning continuation. This is useful for quick midgame decisive imitation probes; keep draw-heavy contrast rows in the same command only when action CE is gated off for those shard paths.
