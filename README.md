...
`adaptive_policy_adapter_gate_supervised.py --feature-model-path <model.eqx>` and `evaluate_adaptive_policy.py --policy-adapter-feature-model-path <model.eqx>` decouple gate features from the policy adapter delta. The adapter checkpoint still supplies the centered policy-logit delta, while the feature model supplies strategy finish and outcome probabilities for the gate. New adapter gates save 18 feature names including draw/win probabilities, normalized time, land/army scoreboard advantage, and contact; old 12/14-feature gate sidecars still load because evaluation slices the feature vector to the saved feature count. `--dataset-format plan-q-prefix` reads executed-prefix shards from `adaptive_plan_q_dataset.py --save-worker-prefix-steps` and labels a positive gate example only when the adapter top action matches a high-advantage accepted prefix action. Use `--min-prefix-plan-advantage`, `--require-prefix-base-not-win`, `--max-prefix-plan-time-to-terminal`, and `--max-prefix-step` to keep the gate focused on decisive enter-plan windows. When training gates for a plain base checkpoint plus a strategy-head adapter, use `--no-base-outcome-head`, `--no-base-strategy-aux`, `--no-base-strategy-spatial-aux`, and `--base-strategy-finish-outputs <n>` to describe only the base model schema; the regular `--outcome-head`/`--strategy-aux` flags still describe the adapter and optional feature model.
