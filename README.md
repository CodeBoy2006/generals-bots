...
`adaptive_online_search_candidate_scorer.py` trains a richer offline scorer over the online-search top-k candidate actions. It expands each candidate into inference-available policy prior, source/destination geometry, local observation features, capture-margin approximations, time, seat, active-area, and visible-enemy features, then learns the search-score ranking. It is currently a diagnostic artifact, not an evaluator path. After removing a leaked `candidate_is_search_action` feature, rpa2 v0+v1 all-changed rows reached val top1 `38.78%` / top2 `70.0%` versus prior top1 `0.38%`; strict `search_converts_to_win` soft-rank rows reached val top1 `55.0%` on only `101` rows, while hard-best CE overfit. This points toward collecting more strict conversion rows and training a planner-aware conditional action head before gameplay integration.
