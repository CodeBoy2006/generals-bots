...
For multi-horizon finishability, use `--finish-head-mode multi-horizon`. This expands the strategy finish head to three independent BCE logits for `finish_within_50`, `finish_within_100`, and a long horizon. The long horizon defaults to 250 for compatibility; pass `--finish-target-horizon 500` when training against the current fixed-v5 max500 gate so the third logit uses `finish_within_500`. Warm-start old binary-finish checkpoints with `--init-finish-head-mode binary`. Evaluation should pass `--strategy-finish-outputs 3`. PPO warm-starts that only need to read and discard the old head can pass `--init-strategy-finish-outputs 3`; PPO runs that should preserve strategy heads in the output checkpoint must also pass `--strategy-aux --strategy-finish-outputs 3`.
