...
`adaptive_plan_pair_supervised.py` 可从 Plan-Q shard 训练显式 source-target pair scorer。它不是 additive `source_logit + target_logit`，而是用 MLP 读取 policy/action-Q delta、source/target logits、finish probability、source/target cell features、坐标、route distance、turn、grid size 和 seat 等推理可得特征，对每行候选 plan 做 row-wise rank CE。脚本会保存最终 checkpoint 和按 validation pair top-1 选择的 `.best.eqx`。第一轮 worker-source + gap-weighting 离线验证强于同 split additive baseline，但混合 model/model-worker shard 和无 gap 小模型都退化，因此当前仍是 Commander-scorer 诊断工具，不直接作为 promotion 推理路径。
