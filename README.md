...
`evaluate_adaptive_policy.py --policy-adapter-path <adapter.eqx> --policy-adapter-scale <x>` loads a second adaptive checkpoint with the same network template and adds its centered legal policy-logit delta to the base policy. `--policy-adapter-finish-threshold <p>` gates that delta on the adapter's strategy finish probability, using a hard on/off gate; multi-horizon finish checkpoints use the last horizon. This is useful for policy-head-only decisive imitation checkpoints that help fixed-v5 but hurt Expander when used as the full policy. The first p0mix adapter probe improved fixed-v5 less than the full p0mix checkpoint but recovered most Expander loss, so it is diagnostic evidence for learned adapter gating rather than a promotion setting.
