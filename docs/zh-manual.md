...
`adaptive_plan_q_dataset.py --candidate-source model --candidate-target model` 会用 checkpoint 的 strategy spatial source/target heads 生成候选 source/target，再分别用可动己方格和 active/passable target mask 约束候选。这比默认 privileged heuristic candidate 更贴近推理时真实会出现的 command 分布，适合后续 gate/Worker 训练；当前 model-candidate shard 的标签质量优于旧 heuristic shard，但直接复用 scalar action-Q replacement gate 仍没有 gameplay 正信号。
