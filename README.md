...
`adaptive_online_search_trace_dataset.py` collects those aligned online-search decisions as ignored `runs/` NPZ shards. It saves the deployment observation, legal mask, base logits/action, executed search action, top-k candidate indices, prior scores, rollout scores, score margin, search-enter flag, weak intent/belief heatmaps, grid size, seat, and turn. `--warmup-steps` advances games before saving so `--search-min-turn 80 --search-require-contact --require-search-used` can target the same midgame distribution that improved fixed-v5 and Expander gates. Initial smoke shards wrote finite trace fields; the first real samples saved `57` Expander 8/12/16 midgame rows with `64.9%` action-change rate and `88` fixed-v5 max500 rows with `56.8%` action-change rate.
