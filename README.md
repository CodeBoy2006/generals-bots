...
`evaluate_adaptive_policy.py --online-search-top-k <k>` enables deployment-style online counterfactual search against a fixed checkpoint opponent. It composes the base policy and optional policy adapter, takes the top-k primitive actions by prior logit, forces each candidate first move, rolls out the same deployment policy against the fixed opponent for `--online-search-rollout-steps` and `--online-search-rollouts-per-action`, then executes the highest-scoring action. The first implementation supports `--opponent-policy-path` gates, not heuristic opponents; use `--online-search-min-turn`, `--online-search-require-contact`, and size gates to keep compute focused on decisive midgame states. Candidate scoring now uses the same sampled opponent first action that the real environment step will execute, which avoids evaluating plans against a different stochastic first response. In the max500 fixed-v5 gate, `v4 + legacy Plan-Q prefix v0 adapter + online search top_k=4 rollout=16 contact-only turn>=80` moved same-seed 512-row min from `41.99%` to `51.56%`, with draw below `7%` on both seats. This is a planner/wrapper result rather than a pure `.eqx` checkpoint, but it is the strongest current fixed-v5 signal and should drive the next distillation targets.
