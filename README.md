...
`adaptive_command_gate_supervised.py` trains an independent binary command-acceptance MLP from Plan-Q shards. It predicts whether to replace the current policy action with the evaluator's source/target worker command using only inference-time features such as policy-logit delta, action-Q delta, source/target logits, finish probability, route distance, source army, and seat. `evaluate_adaptive_policy.py --strategy-command-gate-path <gate.eqx> --strategy-command-gate-threshold <p>` loads the gate. Early model and model-worker candidate probes fit the offline labels, but 256-row fixed-v5 confirmation still regressed, so this is a diagnosis tool until the source/target proposal quality improves.
