...
`adaptive_strategy_supervised.py --balance-strata size-seat` 会在离线训练前按 `(grid_size, seat)` 等量下采样。它适合 high-gap、terminal-window、draw-heavy 这类过滤 shard，避免某个 size/seat 行在主 policy 更新中被隐性放大。
