...
`adaptive_plan_q_supervised.py` 还支持 proposal-map 的 outcome/Q 监督：`--source-q-mse-weight`、`--target-q-mse-weight` 会把 source/target 候选位置的 logits 回归到 `plan_q` 的 source/target 最优值；`--source-q-rank-weight`、`--target-q-rank-weight` 会在候选集合内部做 Q-rank CE；`--q-target-outcome-weight` 可把 decisive plan outcome 按 loss/draw/win 混入 Q target，`--q-rank-temperature` 控制 rank target 锐度。默认权重都是 0，旧 Plan-Q CE/action-Q 命令不受影响。
