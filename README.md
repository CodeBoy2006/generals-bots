...
可设置 `MODEL_PATH=/path/to/model.eqx ./play-v5.command`。脚本会从文件名中的
`augmented` 自动推断 `--policy-input augmented-full-state`；也可以用
...
如果某个 checkpoint 是 18 通道 augmented 输入，脚本会从文件名中的
`augmented` 自动为该玩家传入 `augmented-full-state`。也可以用
`MODEL_0_POLICY_INPUT` 和 `MODEL_1_POLICY_INPUT` 分别覆盖两个 AI 的输入类型。
